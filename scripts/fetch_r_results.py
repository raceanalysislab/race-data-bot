...
BASE = "https://www1.mbrace.or.jp/od2/R"
DOWNLOAD_DIR = "data/download_r"
EXTRACT_DIR = "data/extract_r"
...
def build_url(dt: datetime) -> str:
    return f"{BASE}/{yyyymm(dt)}/r{yymmdd(dt)}.lzh"
...
def already_done(dt: datetime) -> bool:
    ...
    candidates = [
        os.path.join(EXTRACT_DIR, f"r{yy}.txt"),
        os.path.join(EXTRACT_DIR, f"R{yy}.TXT"),
        os.path.join(EXTRACT_DIR, f"R{yy}.txt"),
        os.path.join(EXTRACT_DIR, f"r{yy}.TXT"),
    ]
...
def try_one(dt: datetime):
    yy = yymmdd(dt)
    url = build_url(dt)
    lzh_path = os.path.join(DOWNLOAD_DIR, f"r{yy}.lzh")

    if already_done(dt):
        print(f"exists: r{yy}.txt")
        return True
