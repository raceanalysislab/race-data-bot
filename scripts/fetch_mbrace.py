...
def main():
    ...
    # ✅ ここが修正点：開催判定を「会場名 in text」から「会場ブロックに1R」に変更
    venues = []
    for v in VENUES:
        held = is_held_by_block(best_text, v["name"])
        venues.append({
            "jcd": v["jcd"],
            "name": v["name"],
            "held": held,
